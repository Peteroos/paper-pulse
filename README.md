...
- GitHub Pages 版本只抓取和展示论文数据，不会在 build 时调用 OpenAI；实时 AI 总结和邮件订阅需要一个后端 API，推荐部署到 Vercel
...
Vercel 版本支持用户输入自己的邮箱，并选择感兴趣方向。订阅信息存到 Vercel Blob，Vercel Cron 每天触发 `/api/send-digest`，再通过 Resend 发邮件。

需要在 Vercel 添加这些环境变量：

- `BLOB_READ_WRITE_TOKEN`: 创建或连接 Vercel Blob 后由 Vercel 提供
- `RESEND_API_KEY`: Resend 的 API key
- `RESEND_FROM`: 已验证发件人，例如 `Paper Pulse <papers@yourdomain.com>`

`vercel.json` 里已经配置了每天 `12:30 UTC` 触发邮件任务，约等于美国东部夏令时 `08:30`。如果先只想测试，不要设置 `CRON_SECRET`；设置后需要确保定时请求会带对应的 `Authorization` header。

邮箱、Resend key、Blob token 都只存在 Vercel 服务端环境里。浏览器只会请求 `/api/subscribe` 保存订阅，看不到这些密钥。
...
- 加入退订链接、邮件偏好管理页、飞书或 Telegram 推送
- 增加基于阅读行为的个性化排序
- 增加中文摘要和每日综述
